...
Mix_Chunk *TRUEWAV = nullptr;
Mix_Chunk *FALSEWAV = nullptr;
...
HangmanGame::HangmanGame(WINDOWS* WINDOWS, int time) : window(WINDOWS), playTime(time) {
    ...
    TRUEWAV = Mix_LoadWAV("sound/found.wav");
    FALSEWAV = Mix_LoadWAV("sound/false.wav");
}
...
void HangmanGame::renderGameSDL() {
    ...
    window->createTextTexture("Current Guess   :    " + guessedWord, 100, 800);
    window->createTextTexture("Bad Guesses    :    " + badGuess, 100, 850);
    window->createTextTexture("Hints :   " + to_string(suggested) + "/" + to_string(maxSuggest) + "   (Press 'Space')", 30, 30);
    window->updateScreen();
    SDL_Delay(15);
}
...
void HangmanGame::choosefileNameEvent() {
    ...
    Mix_Chunk *asia = Mix_LoadWAV("sound/Countries Easy.wav");
...
void HangmanGame::choosefileNameEventHard() {
    ...
    Mix_Chunk *asia = Mix_LoadWAV("sound/Countries Hard.wav");
...
void HangmanGame::renderfileNameEasy() {
    ...
    window->createTextTexture("Press 3 : Countries", 150, 200);
...
void HangmanGame::renderfileNameHard() {
    ...
    window->createTextTexture("Press 3 : Countries Hard", 150, 200);
...
HangmanGame::~HangmanGame() {
    if (TRUEWAV) {
        Mix_FreeChunk(TRUEWAV);
        TRUEWAV = nullptr;
    }
    if (FALSEWAV) {
        Mix_FreeChunk(FALSEWAV);
        FALSEWAV = nullptr;
    }
